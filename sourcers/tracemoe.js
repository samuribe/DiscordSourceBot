...
const { createResponseText } = require('../formatting/response');
...
async function handleImageLinkSourcing(url, { minSimilarity, maxAspectRatioError, minColorfulness, minOverhead }) {
    ...
    if (source && source.similarity > minSimilarity && source.imageInfo.frameCountErrorPercent < maxAspectRatioError) {
        ...
        if (info) {
            return {
                source: source,
                responseText: createResponseText(source, info),
            };
        }
    }
    return null;
...
module.exports = { handleImageLinkSourcing };